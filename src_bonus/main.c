...
{	
	...
	x = p->win.x / 2;
	y = p->win.y / 2;
	mlx_put_image_to_window(p->mlx, p->win.win, p->sprite.lose,
		p->pos.lux * SIZE, p->pos.luy * SIZE);
	mlx_string_put(p->mlx, p->win.win, x * SIZE,
		y * SIZE, 0xFF0000, "AVADA KEDAVRA!!");
	p->cont.check_loop = 1;
...
void	get_window(t_all *p)
{
	...
	ft_printf("\033[36m raggiungi la scopa senza toccare Bellatrix\n");
	...
	mlx_loop_hook(p->mlx, ft_enemy, p);
	mlx_loop(p->mlx);
}
